...
class AN1Client:
    ...
    def call(self, input_text: str, max_retries: int = 3) -> Dict[str, Any]:
        """
        Make a request to AN1 API with retry logic.
        
        Generates a deterministic z vector from input text and sends
        it as an infer_z request.
        
        Args:
            input_text: Input text (converted to z vector automatically)
            max_retries: Maximum number of retry attempts
            
        Returns:
            Dict with 'ok', 'status_code', 'api_latency_ms', 'client_elapsed_ms', 'response', 'error'
        """
        # Generate deterministic z vector from text
        z_vector = text_to_z(input_text, dim=256)
        
        # Assert z vector has exactly 256 elements
        if len(z_vector) != 256:
            raise ValueError(
                f"Generated z vector has length {len(z_vector)}, expected exactly 256. "
                f"This indicates a bug in the z vector generator."
            )
