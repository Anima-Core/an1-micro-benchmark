...
This script performs a single `infer_z` request with a fixed 256-dimension vector to validate authentication, endpoint configuration, and API availability. This verifies your API endpoint is accessible and accepts `infer_z` requests before running the full benchmark.
