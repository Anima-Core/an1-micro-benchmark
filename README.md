...
This step is optional but strongly recommended before running the full benchmark. This script performs a single `infer_z` request with a fixed 256-dimension vector to validate authentication, endpoint configuration, and API availability. This verifies your API endpoint is accessible and accepts `infer_z` requests before running the full benchmark.
...
Results are written to `out/summary_<mode>.json` and `out/results_<mode>.csv`,
where `<mode>` is the value of `AN1_EXPECTED_MODE` if set, otherwise `"run"`.
...
- Each request is independent and does not rely on prior state or context
...
The benchmark does not assume task semantics such as classification or generation, and evaluates all requests as stateless inference calls.
